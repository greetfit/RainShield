...
const props = defineProps({ payables: Array });
const table = useTableControls(() => props.payables, ['date', 'reference', 'party', 'total', 'paid', 'due']);
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-100">Supplier Payables</h2>
        </template>

        <div class="py-8">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <TableControls :table="table" placeholder="Search supplier payables...">
                            ...
                            <table class="min-w-[880px] divide-y divide-gray-200 dark:divide-gray-800">
                                <thead class="bg-gray-50 dark:bg-gray-950">
                                    <tr>
                                        ...
                                        <th class="px-5 py-3 text-left text-xs font-semibold uppercase text-gray-500">Supplier</th>
                                ...
                                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                                    <tr v-for="row in table.rows.value" :key="row.id">
                                        ...
                                        <td class="px-5 py-3 text-sm font-medium text-indigo-600 dark:text-indigo-300">
                                            <Link :href="route('purchases.show', row.id)">{{ row.reference }}</Link>
                                        </td>
                                        ...
                                        <td class="px-5 py-3 text-right text-sm font-semibold text-amber-600 dark:text-amber-300">{{ money(row.due) }}</td>
                                    </tr>
                                    <tr v-if="!table.rows.value.length">
                                        <td colspan="6" class="px-5 py-8 text-center text-sm text-gray-500">No supplier dues.</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>

...
<template>
    <Head title="Finished Goods Movements" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-2">
                <Link :href="route('finished-goods.index')" class="text-sm text-indigo-600 hover:underline">Finished Goods</Link>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <tr v-for="m in movements" :key="m.id">
                                ...
                                <td class="px-6 py-3 text-right text-sm text-gray-700">{{ m.quantity }}</td>
                                <td class="px-6 py-3 text-right text-sm text-gray-700">{{ m.balance_quantity }}</td>
                                <td class="px-6 py-3 text-sm text-gray-500">{{ m.note || '-' }}</td>
                            </tr>
                            <tr v-if="movements.length === 0">
                                <td colspan="6" class="px-6 py-8 text-center text-sm text-gray-500">No movements yet.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
